...
// can't rely on anything working until you insert it into a TUISplitView.
...
// These return nil since we're not a TUISplitView (they're overridden there).
...
// Sets and gets the coupling between a TUISplitView and its containing TUISplitView (if any).
...
// RBSplitSubviews are never flipped, unless they're TUISplitViews.
- (BOOL)isFlipped {
	return NO;
}
...
// This returns the owning splitview. It's guaranteed to return a TUISplitView or nil.
// You should avoid having "orphan" RBSplitSubviews, or at least manipulating
// them while they're not inserted in a TUISplitView.
- (TUISplitView*)splitView {
	id result = [self superview];
	if ([result isKindOfClass:[TUISplitView class]]) {
		return (TUISplitView*)result;
	}
	return nil;
}

// This also returns the owning splitview. It's overridden for nested TUISplitViews.
...
// This returns the outermost directly containing TUISplitView, or nil.
- (TUISplitView*)outermostSplitView {
	id result = nil;
	id sv = self;
	while ((sv = [sv superview])&&[sv isKindOfClass:[TUISplitView class]]) {
		result = sv;
	}
	return result;
}

// This convenience method returns YES if the containing TUISplitView is horizontal.
...
// We check if the TUISplitView must be adjusted before redisplaying programmatically.
// if so, we adjust and display the whole TUISplitView.
- (void)display {
	TUISplitView* sv = [self splitView];
	if (sv) {
		if ([sv mustAdjust]) {
...
//- (TUIView *)hitTest:(NSPoint)aPoint {
...
// This does the actual movement.
				[sv _trackMouseEvent:theEvent from:point withBase:base inDivider:thediv];
				if ([sv mustAdjust]) {
...
// Change the drag point by the actual amount moved.
	...
	if (canDragWindow&&[window isMovableByWindowBackground]&&![[self couplingSplitView] backgroundColor]) {
// If we get here, it's a textured (metal) window, the mouse has gone down on an non-opaque portion
// of the subview, and our TUISplitView has a transparent background. TUISplitView returns NO to
...
- (BOOL)RB___stopAnimation {
	if (currentAnimation&&(currentAnimation->owner==self)) {
		...
		free(currentAnimation);
		currentAnimation = NULL;
		[[self splitView] _setDragging:NO];
		return YES;
	}
	return NO;
}
...
// This pair of internal methods is used only inside -[TUISplitView adjustSubviews] to copy subview data
...
- (BOOL)_clearResponder {
	NSWindow* window = [self.nsView window];
	if (window) {
		TUIView* responder = (TUIView*)[window firstResponder];
		if (responder&&[responder respondsToSelector:@selector(isDescendantOf:)]) {
			if ([[responder subviews]containsObject:responder]) {
				if ([window makeFirstResponder:nil]) {
					[responder setNeedsDisplay];
					return YES;
				}
			}
		}
