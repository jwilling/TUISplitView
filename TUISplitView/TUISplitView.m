...
static NSCursor* cursors[TUICursorTypeCount] = {nil};
...
+ (NSCursor*)cursor:(TUICursorType)type {
	if (type<TUICursorTypeCount) {
		NSCursor* result = cursors[type];
		if (result) {
			return result;
		}
		switch (type) {
			case TUIHorizontalCursor:
				return [NSCursor resizeUpDownCursor];
			case TUIVerticalCursor:
...
+ (NSString*)defaultsKeyForName:(NSString*)name isHorizontal:(BOOL)orientation {
    return [NSString stringWithFormat:@"TUISplitView %@ %@",orientation?@"H":@"V",name];
}
...
- (void)setAutosaveName:(NSString*)aString recursively:(BOOL)flag {
	...
	[TUISplitView removeStateUsingName:autosaveName];
	...
	if (flag) {
		...
		for (i=0;i<subcount;i++) {
			TUISplitView* sv = [[subviews objectAtIndex:i] asSplitView];
			if (sv) {
		...
		if ([me isKindOfClass:[NSString class]]) {
			if ([self setStateFromString:me]) {
				...
				for (i=1;i<count;i++) {
					...
					TUISplitView* suv = [[subviews objectAtIndex:i-1] asSplitView];
...
- (void)encodeRestorableStateWithCoder:(NSCoder*)coder {
	...
	[coder encodeObject:[self stringWithSavedState] forKey:@"TUISplitView"];
}

// interface for 10.7 state restoration
- (void)restoreStateWithCoder:(NSCoder*)coder {
	[super restoreStateWithCoder:coder];
	NSString *state = [coder decodeObjectForKey:@"TUISplitView"];
	if(state)
...
// is for nested TUISplitViews is YES; however, isCoupled returns NO if we're not nested.
...
// This returns the containing splitview if they are coupled. It's guaranteed to return a TUISplitView or nil.
...
// TUISplitViews must be flipped to work properly for horizontal dividers. As the subviews are never
...
// Returns YES if the view is directly contained in an TUIScrollView.
- (BOOL)isInScrollView {
...
- (BOOL)isOpaque {
	...
	return sv?[sv isOpaque]:(self.backgroundColor&&([self.backgroundColor alphaComponent]>=1.0));
}
...
// For a nested TUISplitView, orientation is perpendicular to the containing TUISplitView, and
...
// TUISplitViews never resize their subviews automatically.
...
- (void)mouseDown:(NSEvent*)theEvent {
	...
	for (i=0;i<subcount;i++) {
		...
		if (NSPointInRect(where, *divdr)) {
			...
			TUISplitView* trailing = [subviews objectAtIndex:i+1];
			...
			if ([theEvent clickCount]>1) {
				...
				if ([trailing isCollapsed]) {
					if (![leading isCollapsed]) {
						[self _tryToExpandTrailing:trailing leading:leading delta:-[trailing dimension]];
					}
				} else {
					if ([leading isCollapsed]) {
						[self _tryToExpandLeading:leading divider:i trailing:trailing delta:[leading dimension]];
					} else {
						...
						if (lcan) {
							[self _tryToShortenLeading:leading divider:i trailing:trailing delta:-ldim always:NO];
						}
						// If the leading subview didn't collapse for some reason, we try to collapse the trailing one.
						if (!mustAdjust&&tcan) {
							[self _tryToShortenTrailing:trailing divider:i leading:leading delta:[trailing dimension] always:NO];
						}
					}
				}
				...
				[[TUISplitView cursor:TUIDragCursor] push];
				[self _setDragging:YES];
				while ((theEvent = [NSApp nextEventMatchingMask:NSLeftMouseDownMask|NSLeftMouseDraggedMask|NSLeftMouseUpMask untilDate:[NSDate distantFuture] inMode:NSEventTrackingRunLoopMode dequeue:YES])&&([theEvent type]!=NSLeftMouseUp)) {
					...
					[self _trackMouseEvent:theEvent from:where withBase:NSZeroPoint inDivider:i];
					if (ldivdr!=NSNotFound) {
						// Track any two-axis thumbs for the leading nested TUISplitView.
						[leading _trackMouseEvent:theEvent from:[self convertPoint:lwhere toView:leading] withBase:NSZeroPoint inDivider:ldivdr];
					}
					if (tdivdr!=NSNotFound) {
						// Track any two-axis thumbs for the trailing nested TUISplitView.
						[trailing _trackMouseEvent:theEvent from:[self convertPoint:twhere toView:trailing] withBase:NSZeroPoint inDivider:tdivdr];
					}
					if (mustAdjust||[leading mustAdjust]||[trailing mustAdjust]) {
						...
						if ((ldivdr!=NSNotFound)&&![leading isCollapsed]) {
							// Adjust for the leading nested TUISplitView's thumbs while it's not collapsed.
							lrect = [leading RB___dividerRect:ldivdr relativeToView:self];
							OTHER(lwhere) = OTHER(lrect.origin)+loffset;
						}
						if ((tdivdr!=NSNotFound)&&![trailing isCollapsed]) {
							// Adjust for the trailing nested TUISplitView's thumbs while it's not collapsed.
							trect = [trailing RB___dividerRect:tdivdr relativeToView:self];
...
- (void)drawRect:(NSRect)rect {
	...
	for (i=0;i<subcount;i++) {
		...
		if (divdr) {
			// Draw the corresponding two-axis thumbs if the leading view is a nested TUISplitView.
			if ((leading = [leading coupledSplitView])&&lexp) {
				[leading _drawDividersIn:self forDividerRect:dividers[i] thickness:divt];
			}
			// Draw the corresponding two-axis thumbs if the trailing view is a nested TUISplitView.
			if ((trailing = [trailing coupledSplitView])&&texp) {
				[trailing _drawDividersIn:self forDividerRect:dividers[i] thickness:divt];
			}
		}
	}
}
...
- (void)adjustSubviews {
	[self _adjustSubviewsExcepting:nil];
}
...
- (void)resetCursorRects {
	...
	NSInteger i;
	NSCursor* cursor = [TUISplitView cursor:[self isVertical]?TUIVerticalCursor:TUIHorizontalCursor];
	CGFloat divt = [self dividerThickness];
	for (i=0;i<divcount;i++) {
		TUISplitView* sub = [[subviews objectAtIndex:i] coupledSplitView];
		// If the leading subview is a nested TUISplitView, add the thumb rectangles first.
		if (sub) {
			[sub  _addCursorRectsTo:self forDividerRect:dividers[i] thickness:divt];
		}
		sub = [[subviews objectAtIndex:i+1] coupledSplitView];
		// If the trailing subview is a nested TUISplitView, add the thumb rectangles first.
		if (sub) {
			[sub  _addCursorRectsTo:self forDividerRect:dividers[i] thickness:divt];
		}
		...
		if (!NSIsEmptyRect(divrect)) {
			[self.nsView addCursorRect:divrect cursor:cursor];
		}
	}
}

// These two methods encode and decode TUISplitViews. One peculiarity is that we encode the divider image's
// bitmap representation as data; this makes the nib files larger, but the user can just paste any image
// into the TUISplitView inspector - or use the default divider image - without having to include it into the
// project, too.
- (void)encodeWithCoder:(NSCoder *)coder {
	...
	if ([coder allowsKeyedCoding]) {
		...
		[coder encodeObject:self.backgroundColor forKey:@"background"];
...
- (void)_tryToExpandLeading:(RBSplitSubview*)leading divider:(NSUInteger)indx trailing:(RBSplitSubview*)trailing delta:(CGFloat)delta {
	...
	BOOL dowin = ([self _shouldResizeWindowForDivider:indx betweenView:leading andView:trailing willGrow:YES]);
	if (dowin) {
		// We initialize the other local variables only if we need them for the window.
		ishor = [self isHorizontal];
		document = [[[self enclosingScrollView] subviews] objectAtIndex:0];
		if (document) {
			frame = [document frame];
		} else {
			window = [self.nsView window];
			frame = [window frame];
...
- (void)_tryToShortenLeading:(RBSplitSubview*)leading divider:(NSUInteger)indx trailing:(RBSplitSubview*)trailing delta:(CGFloat)delta always:(BOOL)always {
	...
	BOOL ishor = NO;
	// First we ask the delegate, if there's any, if the window should resize.
	BOOL dowin = ([self _shouldResizeWindowForDivider:indx betweenView:leading andView:trailing willGrow:NO]);
	if (dowin) {
		// We initialize the other local variables only if we need them for the window.
		ishor = [self isHorizontal];
		document = [[[self enclosingScrollView] subviews]objectAtIndex:0];
...
- (void)_trackMouseEvent:(NSEvent*)theEvent from:(NSPoint)where withBase:(NSPoint)base inDivider:(NSUInteger)indx {
	...
	if (delta<0.0) {
		...
		if ([firstTrailing isCollapsed]) {
			[self _tryToExpandTrailing:firstTrailing leading:firstLeading delta:delta];
		} else {
			[self _tryToShortenLeading:firstLeading divider:indx trailing:firstTrailing delta:delta always:YES];
		}
	} else if (delta>0.0) {
		...
		if ([firstLeading isCollapsed]) {
			[self _tryToExpandLeading:firstLeading divider:indx trailing:firstTrailing delta:delta];
		} else {
			// The leading subview is not collapsed, so we try to shorten or even collapse it
			[self _tryToShortenTrailing:firstTrailing divider:indx leading:firstLeading delta:delta always:YES];
		}
	}
}
...
- (void)_addCursorRectsTo:(TUISplitView*)masterView forDividerRect:(NSRect)rect thickness:(CGFloat)delta {
	if (dividers) {
		...
		NSInteger i;
		NSCursor* cursor = [TUISplitView cursor:TUI2WayCursor];
		BOOL ishor = [self isHorizontal];
		...
		for (i=0;i<divcount;i++) {
			...
			if (!NSIsEmptyRect(divdr)) {
				[masterView.nsView addCursorRect:divdr cursor:cursor];
			}
		}
	}
}

// This is called for nested TUISplitViews, to draw the two-axis thumbs.
- (void)_drawDividersIn:(TUISplitView*)masterView forDividerRect:(NSRect)rect thickness:(CGFloat)delta {
	if (!dividers) {
		return;
	}
...
// This is usually called from initWithCoder to ensure that the outermost TUISplitView is
// properly adjusted when first displayed.
- (void)_adjustOutermostIfNeeded {
	TUISplitView* sv = [self splitView];
	if (sv) {
		[sv _adjustOutermostIfNeeded];
		return;
	}
	if (mustAdjust&&!isAdjusting) {
		[self adjustSubviews];
...
// The main idea is to always keep the TUISplitView completely covered by dividers and subviews, have at
...
- (void)_adjustSubviewsExcepting:(RBSplitSubview*)excepting {
	...
	while (sanity++<3) {
		...
		for (i=0;i<subcount;i++) {
			...
			if (curr->size>0.0) {
				expsize += curr->size;
				if (!isInScrollView) {
					// ignore fractions if we're in a TUIScrollView, however.
		...
		if (isInScrollView) {
			// If we're inside an TUIScrollView, we just grow the view to accommodate the subviews, instead of
	...
	// newframe is used to reset all subview frames. Subviews always fill the entire TUISplitView along the
	// current orientation.
	...
	// We resize our frame at this point, if we're inside an TUIScrollView.
	if (isInScrollView) {
		[super setFrame:bounds];
	}
	// If there was at least one nested TUISplitView, we loop over the subviews and adjust those that need it.
	for (i=nested;i<subcount;i++) {
		curr = &caches[i];
		TUISplitView* sv = [curr->sub asSplitView];
		if ([sv mustAdjust]) {
			[sv adjustSubviews];
		}
	}
	...
	[[self.nsView window] invalidateCursorRectsForView:self.nsView];
	// Save the state for all subviews.
	if (!isDragging) {
		[self saveState:NO];
	}
	// If we're a nested TUISplitView, also invalidate cursorRects for the superview.
	TUISplitView* sv = [self couplingSplitView];
	if (sv) {
		[[self.nsView window] invalidateCursorRectsForView:self.nsView];
	}
